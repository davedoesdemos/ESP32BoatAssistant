...
char* wifi_scan(void){
    ...
    char *ssid_list = calloc(1, sizeof(char));
    if (!ssid_list) {
        ESP_LOGE(TAG, "Memory allocation failed for SSID list");
        return NULL;
    }
    ...
    for (int i = 0; i < number; i++) {
        ...
        ESP_LOGI(TAG, "Channel \t\t%d", ap_info[i].primary);
                // Calculate size needed: current length + new SSID length + 1 (\n or \0) + 1 (\0 terminator)
        size_t ssid_len = strlen((char *)ap_info[i].ssid);
        size_t new_size = current_len + ssid_len + 2; 

        char *temp = realloc(ssid_list, new_size);
        if (!temp) {
            ESP_LOGE(TAG, "Reallocation failed, returning partial list");
            return ssid_list; 
        }
        ssid_list = temp;
        ...
        // Append the SSID
        strcpy(ssid_list + current_len, (char *)ap_info[i].ssid);
        current_len += ssid_len;

        // Append newline character
        ssid_list[current_len] = '\n';
        current_len++;
        ssid_list[current_len] = '\0';
    }
